...
classdef bch
    ...
    methods
        ...
        function res = gf_poly_eval(obj, poly_coeffs, i) % i is for alpha^i
            ...
            power_set = obj.gf_power_set_indexes(i, size(poly_coeffs,2)-1);
            ...
            power_set_polynomials = obj.gf_ext.Polynomial(power_set, :);
        ...
        function d = calculate_simplified_discrepency(obj, bia_tbl, syndrome, mu)
            ...
            d = syndrome(2*mu+3);
            for i = 1:l
                temp = gfmul(syndrome(2*mu+3-i), sigma_mu(i+1), obj.gf_ext.Polynomial);
                d = gfadd(temp, d, obj.gf_ext.Polynomial);
            end
            return;
        end 
        ...
        function sigma_x = simplified_berlekamp_massey(obj, syndrome) 
            ...
            for mu = 0:obj.t-1
                ...
                if d_mu ~= -Inf
                    ...
                    d_rho_inv = gfdiv(0, d_rho, obj.gf_ext.Polynomial)
                    % multiply to get coeff
                    coeff = gfmul(d_mu, d_rho_inv, obj.gf_ext.Polynomial);
                    % multiply tmp by coeff
                    coeff_resized = zeros(size(tmp)) + coeff
                    tmp
                    tmp = gfmul(tmp, coeff_resized, obj.gf_ext.Polynomial)
                    % add discrepency
                    disp("adding")
                    bia_tbl{mu+3, 2}
                    tmp
                    gfadd(bia_tbl{mu+3, 2}, tmp, obj.gf_ext.Polynomial)
                    bia_tbl{mu+3, 2} = gfadd(bia_tbl{mu+3, 2}, tmp, obj.gf_ext.Polynomial);
        ...
        function res = gf_ext_poly_eval(obj, poly_coeff, i) 
            ...
            % eval terms 
            eval_terms = gfmul(power_set, poly_coeffs_rep, obj.gf_ext.Polynomial)
            % init results 
            res = zeros(length(i), 1) + -Inf;
            % eval each row 
            for i = 1:length(poly_coeff)
                res = gfadd(res, eval_terms(:, i), obj.gf_ext.Polynomial)
            end
            return;
        end
        ...
        function [message, no_error_detected] = decode(obj, r)
            ...
            error_locations = gfdiv(ones(size(root_exps)), root_exps, obj.gf_ext.Polynomial)
            % correct errors
