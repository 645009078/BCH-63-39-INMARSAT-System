...
%                 d {mustBeNumeric} 
%                 t {mustBeNumeric} 
%                 gen_poly(1,:) {mustBeNumeric} % ascending power 
%                 prim_poly(1,:) {mustBeNumeric} % ascending power 
%             end
%             obj.n = n;
%             obj.k = k;
%             obj.m = log2(n + 1);
...
%         end
%         function ps_exponents = gf_power_set_exponents(obj, i, max_pow)
%             i = mod(i, 2^obj.m-1);
...
%             ps_exponents(:, 2) = i'; % x^1 = x
%             pows = 2:max_pow;
%             ps_exponents(:, pows+1) = mod(i' .* pows, 2^obj.m-1);
%         end 
%         % poly coeffs are exponents!
%         function res = gf_poly_eval(obj, poly_coeffs, i) % i is for alpha^i
%             i = mod(i, 2^obj.m-1);
%             
%             poly_degree = size(poly_coeffs, 2) - 1;
%             
%             % exponents for each term in power set [1, alpha^i,
%             % (alpha^i)^2, ....., (alpha^i)^poly_degree]
%             power_set_exponents = obj.gf_power_set_polynomials(i, poly_degree);
%             % put in row major order 
%             power_set_exponents = reshape(power_set_exponents.', 1, []);
%             
%       
%             % repeat polynomial coeffs, once for each value of i
%             rep_poly_coeffs = repmat(poly_coeffs', 1, length(i))';
%             
%             % multiply coefficients with respective values 
%             poly_eval_terms = rep_poly_coeffs .* power_set_exponents;
%             
%             
%             
...
%             intersect(obj.gf_ext.Decimal, si_dec);
%             [~, s_indexs] = ismember(si_dec, obj.gf_ext.Decimal);
%             s_indexs = s_indexs';
%             return;
%         end
...
%             res_exponent = gfmul(a_exponent', b_exponent', obj.gf_ext.Polynomial);
%             res_exponent(res_exponent == -Inf) = -1;
...
% %             res_index = intersect(obj.gf_ext.Decimal, bi2de(res_poly))
%             
%         end
%         function res = add_gf_ext_polynomials(obj, poly_a, poly_b)
%             % poly a and poly b contain indexes in gf_ext table !
%             
...
%           
%             res = arrayfun(@obj.add_gf_ext_elements, p_a, p_b);
%             return;
%         end
...
%                 bia_tbl{mu+3, 2} = bia_tbl{mu+2, 2};                % non-zero discrepency
%                 if d_mu_index ~= 1
...
%             
%             % mult powers with coefficients 
%             eval_term_indexes = arrayfun(@obj.multiply_gf_ext_elements, poly_coeff_indexes, power_set_indexes);
%             
%             res = 1; % zero default
%             for i = 1:length(eval_term_indexes)
